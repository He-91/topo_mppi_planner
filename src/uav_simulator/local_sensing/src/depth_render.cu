...
void DepthRender::set_data(vector<float> &cloud_data)
{
	...
	if(host_cloud_ptr == NULL)
		throw CudaException("DepthRender: unable to allocate host cloud memory.", cudaErrorMemoryAllocation);
...
/*void DepthRender::render_pose( Matrix3d &rotation, Vector3d &translation, int *host_ptr)
{
  ...
  if(err != cudaSuccess)
  	throw CudaException("DepthRender: depth_initial kernel launch failed.", err);
  ...
  err = cudaGetLastError();
  if(err != cudaSuccess)
  	throw CudaException("DepthRender: render kernel launch failed.", err);
