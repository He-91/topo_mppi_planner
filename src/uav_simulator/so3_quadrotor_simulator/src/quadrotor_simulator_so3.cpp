...
main(int argc, char** argv)
{
  ...
  // Hold the vehicle at its initial altitude until the controller publishes
  // the first SO3 command. Benchmark launch files intentionally start the
  // simulator before the planner; without this gravity compensation the drone
  // falls to z=0 during startup and immediately triggers planner Z-safety.
  command.force[0] = 0.0;
  command.force[1] = 0.0;
  ...
  command.kOm[1] = 0.15;
  command.kOm[2] = 0.15;
  command.corrections[0] = 0.0;
  command.corrections[1] = 0.0;
  command.corrections[2] = 0.0;
  command.current_yaw = 0.0;
