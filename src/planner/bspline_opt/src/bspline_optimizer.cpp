...
#include "ddo_bspline_opt/gradient_descent_optimizer.h"
// using namespace std;
...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/max_vel_z", max_vel_z_, max_vel_);
    nh.param("optimization/max_acc_z", max_acc_z_, max_acc_);
                                             ...
                                             Eigen::MatrixXd &gradient)
  {
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      // Z velocity constraint (j=2) - use independent limit
      if (vi(2) > max_vel_z_)
      {
        cost += pow(vi(2) - max_vel_z_, 2) * ts_inv2;
        gradient(2, i + 0) += -2 * (vi(2) - max_vel_z_) / ts * ts_inv2;
        gradient(2, i + 1) += 2 * (vi(2) - max_vel_z_) / ts * ts_inv2;
      }
      else if (vi(2) < -max_vel_z_)
      {
        cost += pow(vi(2) + max_vel_z_, 2) * ts_inv2;
        gradient(2, i + 0) += -2 * (vi(2) + max_vel_z_) / ts * ts_inv2;
        gradient(2, i + 1) += 2 * (vi(2) + max_vel_z_) / ts * ts_inv2;
    ...
    for (int i = 0; i < q.cols() - 2; i++)
    {
      ...
      // XY acceleration constraints (j=0,1)
      for (int j = 0; j < 2; j++)
      {
        if (ai(j) > max_acc_)
        {
      ...
      // Z acceleration constraint (j=2) - use independent limit
      if (ai(2) > max_acc_z_)
      {
        cost += pow(ai(2) - max_acc_z_, 2);
        gradient(2, i + 0) += 2 * (ai(2) - max_acc_z_) * ts_inv2;
        gradient(2, i + 1) += -4 * (ai(2) - max_acc_z_) * ts_inv2;
        gradient(2, i + 2) += 2 * (ai(2) - max_acc_z_) * ts_inv2;
      }
      else if (ai(2) < -max_acc_z_)
      {
        cost += pow(ai(2) + max_acc_z_, 2);
        gradient(2, i + 0) += 2 * (ai(2) + max_acc_z_) * ts_inv2;
        gradient(2, i + 1) += -4 * (ai(2) + max_acc_z_) * ts_inv2;
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        if (j < 0) // fail to get the obs free point
        {
          ...
          //  FIX: Safely terminate instead of continuing with in_id=0
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    // V18 FIX: Increased restart limit 3→5 to give more chances for collision resolution
    constexpr int MAX_RESART_NUMS_SET = 5;
    //  IMPROVED: Increased max rebound times 20→30 for complex scenarios
    constexpr int MAX_REBOUND_TIMES = 30;
    
    //  恢复原始ego-planner的do-while循环逻辑（支持最多30次rebound）
    do
    {
      /* ---------- prepare ---------- */
      ...
      //  IMPROVED: Increased max_iterations 200→300 for better convergence
      lbfgs_params.max_iterations = 300;
      //  IMPROVED: Relaxed convergence threshold 0.01→0.015 to allow earlier stopping
      lbfgs_params.g_epsilon = 0.015;
      
      // V18 FIX: On restart, tighten convergence and allow more iterations
      // to prevent false convergence caused by large lambda2
      if (restart_nums > 0) {
        lbfgs_params.g_epsilon = 0.001;   // Tighter convergence threshold
        lbfgs_params.max_iterations = 500; // More iterations for harder problems
      }
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          ...
          if (flag_occ)
          {
            ...
            {
              //  V8 FIX: Instead of immediately returning false, try to push
              // the first few control points away from obstacles using EDT gradient.
              // This breaks the "stuck hovering" loop where B-spline fails repeatedly
              // because the drone position is at the obstacle inflation boundary.
              bool rescued = false;
              if (grid_map_ != nullptr)
              {
                for (int cp_idx = 1; cp_idx <= std::min(4, (int)cps_.points.cols() - 1); ++cp_idx)
                {
                  Eigen::Vector3d pt = cps_.points.col(cp_idx);
                  Eigen::Vector3d grad;
                  double dist = grid_map_->getDistanceWithGrad(pt, grad);
                  if (dist < dist0_)  // dist0_ is the clearance threshold
                  {
                    if (grad.norm() > 1e-6)
                    {
                      // Push control point along EDT gradient to reach safe distance
                      double push_dist = dist0_ - dist + 0.05;  // Extra 5cm margin
                      cps_.points.col(cp_idx) = pt + grad.normalized() * push_dist;
                    }
                  }
                }
                // Re-check after pushing
                UniformBspline traj_check = UniformBspline(cps_.points, 3, bspline_interval_);
                double tm_c, tmp_c;
                traj_check.getTimeSpan(tm_c, tmp_c);
                bool still_occ = false;
                for (double t_c = tm_c; t_c < tm_c + bspline_interval_; t_c += t_step)
                {
                  if (grid_map_->getInflateOccupancy(traj_check.evaluateDeBoorT(t_c)))
                  {
                    still_occ = true;
                    break;
                  }
                ...
                {
                  rescued = true;
                  ROS_INFO("[B-spline] Rescued first control points from obstacles via EDT gradient push");
                  // flag_occ remains true → outer code enters "restart" branch → re-optimizes
                  break;  // Break out of collision check loop
                }
              }
        ...
        else // restart
        {
          ...
          // V18 FIX: Gentler lambda2 growth (×1.5 instead of ×2) to prevent L-BFGS false convergence
          new_lambda2_ *= 1.5;
      ...
      {
        //  恢复原始逻辑：允许rebound重试（最多30次）
        flag_force_return = true;
        rebound_times++;
        
        //  IMPROVED: Adaptive lambda cooling - reduce collision penalty after excessive rebounds
        if (rebound_times > 10 && rebound_times % 5 == 0) {
          new_lambda2_ *= 0.7;  // Reduce collision penalty by 30%
          ROS_WARN("[B-spline] Excessive rebounds (%d), cooling lambda2: %.3f", rebound_times, new_lambda2_);
        }
      ...
      else
      {
        ...
        // while (ros::ok());
             ...
             && (ros::Time::now() - t0).toSec() < 2.0);  // Total time limit: 2 seconds
