...
#include <ddo_path_searching/mppi_dynamics.h>
#include <ddo_path_searching/mppi_cost.h>
#include <ddo_path_searching/mppi_sampling.h>
#include <vector>
#include <random>
#include <memory>
#include <chrono>
...
private:
    ...
    bool use_batch_iterative_update_{false};
    int gpu_random_seed_{0};
    double gpu_nominal_max_vel_{-1.0};
    double gpu_nominal_max_acc_{-1.0};
    ...
    // Internal helpers (extracted to eliminate duplication)
    bool validatePlannerState() const;
#ifdef USE_GPU_MPPI
    void uploadEDTToGPU();
    
    /**
     * @brief Prepare EDT for GPU — call once per replan cycle, NOT per topo path.
     * Converts double→float and uploads to GPU only if EDT data has changed.
     */
    void prepareEDTForGPU();
    
    bool gpuPlan(const Eigen::Vector3d& start_pos,
    ...
    // EDT GPU upload caching — avoid redundant conversion & upload within one replan cycle
    std::vector<float> edt_float_cache_;      // Cached float EDT buffer (reuse allocation)
    const double* edt_last_ptr_;              // Last uploaded esdf_buffer pointer (detect changes)
    size_t edt_last_size_;                    // Last uploaded grid size
    bool edt_gpu_ready_;                      // EDT already uploaded in current replan cycle
#endif
    
    // Rollout functions (thread-safe, for OpenMP parallel execution)
...
force_cpu_ = force;
    ...
    /**
     * @brief Prepare EDT data for GPU before a replan cycle.
     * Call this ONCE before the topo-path loop. Each gpuPlan() will skip re-upload.
     * After the replan cycle completes, call markEDTDirty() to allow next upload.
     */
    void beginReplanCycle() {
#ifdef USE_GPU_MPPI
        if (use_gpu_ && gpu_planner_) {
            edt_gpu_ready_ = false;  // Allow one upload at the start of this cycle
        }
#endif
    }
    
    /**
     * @brief Mark EDT as needing re-upload on next replan cycle.
     * Called automatically at the start of beginReplanCycle().
     */
    void endReplanCycle() {
#ifdef USE_GPU_MPPI
        // edt_gpu_ready_ stays true until beginReplanCycle() resets it
#endif
    }
    ...
    bool planTrajectoryBatch(const Eigen::Vector3d& start_pos,
                             const Eigen::Vector3d& start_vel,
                             const Eigen::Vector3d& goal_pos,
                             const Eigen::Vector3d& goal_vel,
                             const std::vector<std::vector<Eigen::Vector3d>>& initial_paths,
                             std::vector<MPPITrajectory>& optimal_trajectories);
    ...
    void setVehicleLimitsZ(double max_vel_z, double max_acc_z);
    
    /**
     * @brief  设置动态障碍物预测数据 (GPU + CPU)
     * @param positions Flattened predicted positions [obs0_t0, obs0_t1, ..., obs1_t0, obs1_t1, ...]
     * @param radii Obstacle radii [num_obstacles]
     * @param num_obstacles Number of dynamic obstacles
     * @param horizon Number of prediction timesteps per obstacle
    ...
    /**
     * @brief  设置当前动态障碍物状态 (CPU cost function)
     * @param current_positions Current obstacle positions [num_obstacles]
     * @param radii Obstacle radii [num_obstacles]
     */
    void setDynamicObstaclesCurrent(const std::vector<Eigen::Vector3d>& current_positions,
