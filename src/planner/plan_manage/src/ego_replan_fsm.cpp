...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    bspline_pub_ = nh.advertise<topo_mppi_planner::Bspline>("/planning/bspline", 10);
    data_disp_pub_ = nh.advertise<topo_mppi_planner::DataDisp>("/planning/data_display", 100);
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      // 🔥 P2: Trajectory tracking error feedback (SAFE version - only monitoring)
      try {
        Eigen::Vector3d pos_error, vel_error;
        if (planner_manager_->computeTrackingError(odom_pos_, odom_vel_, time_now.toSec(), 
                                                   pos_error, vel_error)) {
          // Compute PD feedback correction
          Eigen::Vector3d correction = planner_manager_->feedbackCorrection(pos_error, vel_error);
          
          // Log tracking error (throttled to 1Hz)
          ROS_INFO_THROTTLE(1.0, "[P2 Tracking] Error: pos=%.3fm, vel=%.3fm/s, correction=%.3fm/s²",
                           pos_error.norm(), vel_error.norm(), correction.norm());
          
          // 🛡️ DISABLED: Replan triggering - too aggressive for now
          // if (planner_manager_->shouldReplanFromError(pos_error)) {
          //   ROS_WARN("[P2 Tracking] ⚠️ Large tracking error %.3fm > threshold, triggering replan",
          //            pos_error.norm());
        ...
        ROS_WARN_THROTTLE(5.0, "[P2 Tracking] Exception: %s", e.what());
      }
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    if (plan_success)
    {
      ...
      /* publish traj */
      topo_mppi_planner::Bspline bspline;
      bspline.order = 3;
