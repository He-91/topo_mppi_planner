...
#include <ddo_plan_env/raycast.h>
...
public:
  ...
  //  NEW: GPU MPPI EDT data access
  struct EDTData {
    const double* esdf_buffer;      // ESDF data pointer
    const double* esdf_buffer_neg;  // Negative ESDF data pointer
    int size_x, size_y, size_z;     // Grid dimensions
    double resolution;              // Grid resolution
    double origin_x, origin_y, origin_z;  // Grid origin
  };
  EDTData getEDTData() const;
